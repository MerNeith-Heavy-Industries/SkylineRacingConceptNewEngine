    ...
    public IXamlAstNode Transform(AstTransformationContext context, IXamlAstNode node)
    {
        ...
        if (node is XamlManipulationGroupNode group)
        {
            group.Children.RemoveAll(n => n is XamlAstXmlDirective directive &&
                (directive.Name == "Class" || directive.Name == "Key" || directive.Name == "Name"));
        }
...
public class CompileXamlTask : Task
{
    ...
    private XamlLanguageTypeMappings CreateTypeMappings(CecilTypeSystem typeSystem)
    {
        ...
        TryAddType(typeSystem, "nfm_world.ui.yoga.xaml.XmlnsDefinitionAttribute", mappings.XmlnsAttributes);
        TryAddType(typeSystem, "nfm_world.ui.yoga.xaml.ContentPropertyAttribute", mappings.ContentAttributes);
        TryAddType(typeSystem, "nfm_world.ui.yoga.xaml.WhitespaceSignificantCollectionAttribute", mappings.WhitespaceSignificantCollectionAttributes);
        TryAddType(typeSystem, "nfm_world.ui.yoga.xaml.TrimSurroundingWhitespaceAttribute", mappings.TrimSurroundingWhitespaceAttributes);
        TryAddType(typeSystem, "nfm_world.ui.yoga.xaml.UsableDuringInitializationAttribute", mappings.UsableDuringInitializationAttributes);
        TryAddType(typeSystem, "nfm_world.ui.yoga.xaml.DeferredContentAttribute", mappings.DeferredContentPropertyAttributes);

        // Set up our runtime interfaces
        var rootObjectProvider = typeSystem.FindType("nfm_world.ui.yoga.xaml.IRootObjectProvider");
        if (rootObjectProvider != null)
            mappings.RootObjectProvider = rootObjectProvider;

        var uriContext = typeSystem.FindType("nfm_world.ui.yoga.xaml.IUriContext");
        if (uriContext != null)
            mappings.UriContextProvider = uriContext;

        var provideValueTarget = typeSystem.FindType("nfm_world.ui.yoga.xaml.IProvideValueTarget");
        if (provideValueTarget != null)
            mappings.ProvideValueTarget = provideValueTarget;

        var addChild = typeSystem.FindType("nfm_world.ui.yoga.xaml.IAddChild");
        if (addChild != null)
            mappings.IAddChild = addChild;

        var addChildOfT = typeSystem.FindType("nfm_world.ui.yoga.xaml.IAddChild`1");
        if (addChildOfT != null)
