...
public class LuaGamemodeFactory : BaseGamemodeFactory
{
    ...
    public LuaGamemodeFactory(string gamemodeId, IReadOnlyDictionary<string, object>? config = null)
    {
    ...
    public override LuaGamemode CreateGameMode(ClientGamemodeParameters parameters, IGamemodeContext gamemodeContext)
    {
        return _radpack != null
            ? new LuaGamemode(parameters, gamemodeContext, _gamemodeId, _radpack, _config)
            : new LuaGamemode(parameters, gamemodeContext, _gamemodeId, _config);
    }

    public override LuaServerGamemode CreateServerGamemode(ServerGamemodeParameters parameters, IServerGamemodeData data)
    {
        return _radpack != null
            ? new LuaServerGamemode(parameters, data, _gamemodeId, _radpack, _config)
            : new LuaServerGamemode(parameters, data, _gamemodeId, config: _config);
    }
}
