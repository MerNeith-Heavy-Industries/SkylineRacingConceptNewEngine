...
/// </summary>
public class StyleSheet : IList<KeyValuePair<object, object?>>,
    IDictionary<object, object?>, INotifyCollectionChanged, INotifyPropertyChanged
 {
    ...
    private readonly List<StyleSheet> _mergedStyleSheets = [];

    public IList<StyleSheet> MergedStyleSheets => _mergedStyleSheets;
    ...
    public bool TryGetValue(object key, out object? value)
    {
        ...
        for (int i = _mergedStyleSheets.Count - 1; i >= 0; i--)
        {
            if (_mergedStyleSheets[i].TryGetValue(key, out value))
                return true;
        }
        value = null;
        return false;
    ...
    public ICollection<object> Keys
    {
        get
        {
            ...
            for (int i = _mergedStyleSheets.Count - 1; i >= 0; i--)
            {
                foreach (var k in _mergedStyleSheets[i].Keys)
                    keys.Add(k);
            }
    ...
    public ICollection<object?> Values
    {
        get
        {
            ...
            for (int i = _mergedStyleSheets.Count - 1; i >= 0; i--)
            {
                foreach (var v in _mergedStyleSheets[i].Values)
                    values.Add(v);
            }
    ...
    public int Count => _entries.Count + _mergedStyleSheets.Sum(d => d.Count);
