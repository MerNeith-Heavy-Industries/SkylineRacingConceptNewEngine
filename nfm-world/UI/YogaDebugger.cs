...
{
    public static IEnumerable<Node> YogaRootsThisFrame => NodeDebugger.YogaRoot is { } root ? [root] : [];
    ...
    private static void RenderPage3()
    {
        ...
        var maxDepth = 0;
        foreach (var root in YogaRootsThisFrame)
        {
            maxDepth = Math.Max(maxDepth, GetMaxDepth(root, 0));
            continue;

            static int GetMaxDepth(Node visual, int depth)
            {
                var childMax = depth;
        ...
        foreach (var root in YogaRootsThisFrame)
        {
            ...
            y += 24 * (1 + (root is View box ? GetChildCount(box) : 0));
        }

        return;

        void DrawElementAndChildren(Node visual, int y = 15, int depth = 0)
        {
            if (visual is Component node)
            {
            ...
            foreach (var child in visual.VisualChildren)
            {
                ...
                y += 24 * (1 + (child is View childBox ? GetChildCount(childBox) : 0));
            }
        }
        
        static int GetChildCount(View child)
        {
            var count = child.Children.Count;
            foreach (var grandChild in child.Children)
            {
                if (grandChild is View box)
    ...
    private static void RenderPage2()
    {
        ...
        foreach (var root in YogaRootsThisFrame)
        {
            ...
            void DrawNodeAndChildren(Node visual, int depth)
            {
                if (visual is Component node)
                {
                    var color = Color.Lerp(Color.Red, Color.Yellow, depth / (float)maxDepth);
    ...
    {
        var mouseOverNodeTree = YogaRootsThisFrame
        ...
        if (mouseOverNodeTree?.Length > 0)
        {
            for (int i = 0; i < mouseOverNodeTree.Length; i++)
            {
                ...
                var info = $"Component: {node.Name ?? ""}[{node.GetType().Name}] from {(debugInfo.CtorCallerFilePath != "" ? Path.GetRelativePath(slnDir, debugInfo.CtorCallerFilePath) : "")}:{debugInfo.CtorCallerMemberName}:{debugInfo.CtorCallerLineNumber}";
    ...
    private static Component[] FindMouseOverNodeTree(Node visual)
    {
