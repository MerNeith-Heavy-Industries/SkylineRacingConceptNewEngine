...
public class ReactorComponentFactoryGenerator : IIncrementalGenerator
{
    ...
    private void GenerateComponentClasses(SourceProductionContext spc, System.Collections.Immutable.ImmutableArray<ComponentTypeInfo?> types)
    {
        ...
        foreach (var type in nonNull)
        {
            ...
            GenerateComponentFactory(sbFactories, type);
        }
    ...
    private static void GenerateComponentFactory(IndentedStringBuilder sb, ComponentTypeInfo type)
    {
        var nodeName = type.ShortName + "Node";
        ...
        {
            sb.AppendLine();
            sb.AppendLine($"/// <summary>Factory methods for <see cref=\"{type.ShortName}\"/> component nodes.</summary>");
            sb.AppendLine($"public static class {type.ShortName}ComponentFactories");
            sb.AppendLine("{");
            using (sb.Indent())
            {
                sb.AppendLine($"/// <summary>Create a <see cref=\"{type.ShortName}\"/> component VNode.</summary>");
                sb.Append($"public static global::{type.Namespace}.{nodeName} {type.ShortName}(");
                ...
                // Emit parameters with line breaks for readability
                sb.AppendLine();
                using (sb.Indent())
                {
                ...
                using (sb.Indent())
                {
                    sb.AppendLine($"var n = new {type.Namespace}.{nodeName}();");
                    sb.AppendLine();
