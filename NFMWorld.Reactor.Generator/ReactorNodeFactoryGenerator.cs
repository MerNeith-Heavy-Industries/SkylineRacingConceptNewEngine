...
public class ReactorNodeFactoryGenerator : IIncrementalGenerator
{
    ...
    private static TypeInfo? CollectTypeInfo(INamedTypeSymbol symbol)
    {
        ...
        // that are marked with [Property] and fields of type Property<T>.
        {
            var current = symbol;
            while (current is not null)
            {
                foreach (var member in current.GetMembers())
                {
                    ...
                    {
                        // Detect Property<T> fields: e.g. "internal Property<float> _opacity"
                        var fieldTypeName = field.Type.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat);
                        if (!fieldTypeName.Contains(PropertyTGenericFqn)) continue;

                        // Extract T from Property<T>
    ...
    private static void GenerateNodeClass(IndentedStringBuilder sb, TypeInfo type, Dictionary<string, string> typeToNodeClass, List<TypeInfo> allTypes)
    {
        ...
        // Separate Property<T> fields from regular [Property] properties
        var propertyTProps = type.Properties.Where(p => p.IsPropertyT).ToList();
        var regularProps = type.Properties.Where(p => !p.IsPropertyT).ToList();
        ...
        using (sb.Indent())
        {
            // Only generate snapshot for regular [Property] properties
            if (regularProps.Count > 0)
            {
                sb.AppendLine("private class PropertySnapshot : global::NFMWorld.Reactor.BasePropertySnapshot");
                sb.AppendLine("{");
                using (sb.Indent())
                {
                    ...
                    using (sb.Indent())
                    {
                        sb.AppendLine($"var typedVisual = ({type.FullName})visual;");
                        foreach (var prop in regularProps)
                        {
                            var pascalName = PascalCase(prop.Name);
                            sb.AppendLine($"if ({pascalName}.HasValue) typedVisual.{prop.Name} = {pascalName}.Value;");
                        }
                    }
                    sb.AppendLine("}");
                
                    sb.AppendLine("public override void ClearProperties()");
                    sb.AppendLine("{");
                    using (sb.Indent())
                    {
                        foreach (var prop in regularProps)
                        {
                            var pascalName = PascalCase(prop.Name);
                            sb.AppendLine($"{pascalName} = default;");
                        }
                    }
                    sb.AppendLine("}");
                }
            ...
            // Generate backing fields for declared properties
            ...
            // Generate With* for all hierarchy properties
            foreach (var prop in type.Properties)
            {
                var camelName = CamelCase(prop.Name);
            ...
            // Shadow common VNode members with correct return type
            ...
            if (!type.IsAbstract)
            {
                ...
                using (sb.Indent())
                {
                    // Snapshot handling for regular [Property] properties
                    if (regularProps.Count > 0)
                    {
                        sb.AppendLine("PropertySnapshot snapshot;");
                        sb.AppendLine("if (propertySnapshot is null) { snapshot = new PropertySnapshot(); propertySnapshot = snapshot; }");
                        sb.AppendLine("else { snapshot = (PropertySnapshot)propertySnapshot; }");
                        sb.AppendLine();
                    }
                    sb.AppendLine($"var typedVisual = ({type.FullName})visual;");

                    // Property<T> fields: assign or clear OverrideValue (handles staleness)
                    foreach (var prop in propertyTProps)
                    {
                        var camelName = CamelCase(prop.Name);
                        if (prop.IsValueType)
                        {
                    ...
                    // Regular [Property] properties: snapshot save + assign
                    foreach (var prop in regularProps)
                    {
                        var camelName = CamelCase(prop.Name);
                        var pascalName = PascalCase(prop.Name);
